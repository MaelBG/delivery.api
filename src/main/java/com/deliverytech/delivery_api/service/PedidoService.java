...
/**
    ...
    public Pedido atualizarStatus(Long pedidoId, StatusPedido novoStatus) {
        ...
        // Adicione validações conforme necessário para a transição de status
        // Exemplo:
        if (pedido.getStatus() == StatusPedido.CANCELADO) {
            throw new IllegalArgumentException("Não é possível alterar o status de um pedido cancelado.");
        }
        if (pedido.getStatus() == StatusPedido.ENTREGUE && novoStatus != StatusPedido.ENTREGUE) {
            throw new IllegalArgumentException("Pedido já entregue não pode ter o status alterado para um estado anterior.");
        }
