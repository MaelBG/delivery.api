...
public class PedidoController {
    ...
    @Operation(summary = "Adicionar item ao pedido", description = "Adiciona um produto a um pedido existente.", security = @SecurityRequirement(name = "Bearer Authentication"))
    @ApiResponses({
    ...
    @PreAuthorize("hasRole('CLIENTE') or hasRole('ADMIN')")
    @Operation(summary = "Confirmar pedido", description = "Confirma um pedido pendente.", security = @SecurityRequirement(name = "Bearer Authentication"))
    @ApiResponses({
    ...
    @Operation(summary = "Listar pedidos", description = "Lista pedidos com filtros opcionais (status, data inicial e data final) e paginação.", security = @SecurityRequirement(name = "Bearer Authentication"))
    @ApiResponses({
    ...
    public ResponseEntity<PagedResponseWrapper<PedidoResponseDTO>> listar(
            ...
            @Parameter(description = "Data inicial para filtro de período (formato YYYY-MM-DD).") @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate dataInicio,
            @Parameter(description = "Data final para filtro de período (formato YYYY-MM-DD).") @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate dataFim,
    ...
    @Operation(summary = "Atualizar status do pedido", description = "Atualiza o status de um pedido, respeitando as transições válidas.", security = @SecurityRequirement(name = "Bearer Authentication"))
    @ApiResponses({
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('CLIENTE') or hasRole('ADMIN')")
    @Operation(summary = "Cancelar pedido", description = "Cancela um pedido se possível (não pode ser cancelado se já entregue).", security = @SecurityRequirement(name = "Bearer Authentication"))
    @ApiResponses({
    ...
    @Operation(summary = "Histórico do cliente", description = "Lista todos os pedidos de um cliente específico.", security = @SecurityRequirement(name = "Bearer Authentication"))
    @ApiResponses({
    ...
    @Operation(summary = "Pedidos do restaurante", description = "Lista todos os pedidos de um restaurante, com filtro de status opcional.", security = @SecurityRequirement(name = "Bearer Authentication"))
    @ApiResponses({
    ...
    public ResponseEntity<ApiResponseWrapper<CalculoPedidoResponseDTO>> calcularTotal(
        ...
        ApiResponseWrapper<CalculoPedidoResponseDTO> response = new ApiResponseWrapper<>(true, calculo,
